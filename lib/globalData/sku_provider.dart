...
import 'package:myappflutter/util/sku_util.dart';
...
class SkuProvider with ChangeNotifier {
  ...
  void addChoiceNum(choiceNumVal){
    this.choiceNum = choiceNumVal;
    print("addChoiceNum${this.choiceNum}");
    //延时任务会重复执行
    //  Future.delayed(Duration(milliseconds: 10)).then((e) {
    //  });
    Timer t = Timer(Duration(seconds: 500), () {
        notifyListeners();
    });
...
class OrderSkuProvider with ChangeNotifier{
  List<OrderSkuQueryModel> skusQuery=[];
  void addSkusQuery(skus){
    this.skusQuery = skus;
    notifyListeners();
  }
  void addSkuQuery(sku){
    this.skusQuery.add(sku);
    notifyListeners();
  }
}