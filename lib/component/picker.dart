...
class PickerItem {
  ...
  final int areaId;
  final int parentId;
  List<PickerItem> child=[];
  upChild(List<PickerItem> data){
    this.child=data;
  }
  PickerItem(this.text, {this.areaId,this.parentId,this.child});
  toChildJson(child){
    List <Map<String,dynamic>> jihe;
    for(var pick in child){
        jihe.add(
          <String, dynamic>{
            'text': pick.text,
            'parentId':pick.parentId,
            'areaId': pick.areaId,
            'child':toChildJson(pick.child).toString()
          }
        );
    }
    return jihe;
